...
**Key SDK Components:**
  ...
  * **`ms.InitWithEnv(ctx, "", &env)`**: This function is the SDK's entry point for initialization. It sets up:
      * **Structured Logging (`slog`)**: Configures logging based on `LOG_LEVEL`.
...
:::warning
Keep the `go` directive in your `go.mod` no higher than the Go version of this Dockerfile's base image (this guide uses `golang:1.25`). If you run `go mod init` / `go mod tidy` with a newer local Go, the directive is stamped to that version and the container build fails with `go.mod requires go >= 1.25.0 (running go 1.23.7)`. Either pin the `go` directive to match the image or bump the base image.
:::
...
5.  **Start Collector**: From your collector's directory (`opendatahub-collectors/collectors/s3-poller`), run its `docker-compose.yml`:
    ...
    docker compose --profile bdp up --build # Build and run your collector
    ```
