---
sidebar_position: 4
slug: /content-api-transformers
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Transformers for the Content API

A transformer turns raw collected data into Open Data Hub entities. Time series transformers push measurements to the Timeseries Writer with `go-bdp-client` (see [Developing a Data Transformer from Scratch](/data-ingestion/data-transformer-from-scratch)). When your data is structured **content** rather than measurements (for example announcements, points of interest, or events), you push it to the **Content API** instead, using the `clib` package of the `opendatahub-go-sdk`.

This guide walks through that pattern, using the `traffic-event-a22-opendata` transformer (which turns A22 motorway events into Content API `Announcement` entities) as the reference implementation.

## 1. When to use the Content API

- **Time series** measurements from stations and sensors go to the Timeseries Writer with `go-bdp-client`. See [Developing a Data Transformer from Scratch](/data-ingestion/data-transformer-from-scratch).
- **Structured content** entities (announcements, points of interest, events, and similar) go to the Content API with `clib`.

Both kinds of transformer share the same ingestion plumbing: they consume raw data from the message queue with `ms.InitWithEnv` and `tr.NewTr`. Only the write side differs.
...
## 3. Stateful memory with the cache (optional)
...
The listener is the same one every transformer uses; the right way to consume the raw data depends on **how the collector stored it**. `tr.NewTr[string]` together with `RawString2JsonMiddleware` is correct when the collector saved the raw payload as a serialized JSON string (a common case). If the collector stored structured data, consume it directly with `tr.NewTr[YourDTO]` and no middleware. See [Developing a Data Transformer from Scratch](/data-ingestion/data-transformer-from-scratch) for that distinction. Only the body of `Transform` and the `clib` write calls are specific to the Content API.
...
- [Developing a Data Transformer from Scratch](/data-ingestion/data-transformer-from-scratch) (Timeseries Writer)
