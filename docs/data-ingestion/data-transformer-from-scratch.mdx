...
1.  Fetch an access token from the `BDP_TOKEN_URL` using the provided client credentials.
...
**Key SDK Integration Points:**
  ...
  * **`bdplib.FromEnv()`**: This function from the `go-bdp-client` simplifies the initialization of the BDP client. It automatically reads environment variables prefixed with `BDP_` to configure the base URL, provenance details, and OAuth2 authentication (token URL, client ID/secret). This centralizes authentication logic.
...
envSecretRef: # Reference Kubernetes secrets for sensitive information
  - name: MQ_URI
    ...
    key: uri
  - name: BDP_TOKEN_URL # OAuth token URL from secret
    secret: odh-oauth-client-credentials
    key: token_url
  - name: BDP_CLIENT_ID # OAuth Client ID from secret
    secret: odh-oauth-client-credentials
    key: client_id
  - name: BDP_CLIENT_SECRET # OAuth Client Secret from secret
    secret: odh-oauth-client-credentials
    key: client_secret
...
**Critical Points (Unique to Transformer / OAuth):**
  ...
  * **OAuth Credentials in Secrets**: For production, `BDP_CLIENT_ID`, `BDP_CLIENT_SECRET`, and `BDP_TOKEN_URL` should **always** be loaded from Kubernetes secrets using `envSecretRef`. This ensures sensitive authentication details are not committed to Git.
...
2.  **Navigate to Transformer Directory**: `cd opendatahub-collectors/transformers/foo-bar` (or your specific transformer's path).
3.  **Create `.env`**: Copy the provided `.env` content into a file named `.env` in this directory and set the required values: the `MQ_*` queue settings, the Timeseries Writer OAuth credentials (see [section 3.3](#33-authentication-for-the-timeseries-writer)), and `RAW_DATA_BRIDGE_ENDPOINT`.
4.  **Start Infrastructure**: From the `infrastructure-v2` directory, run the base and timeseries compose files:
    ...
    ```
5.  **Start Transformer**: From your transformer's directory (`opendatahub-collectors/transformers/foo-bar`), run its `docker-compose.yml`:
    ```bash
    cd opendatahub-collectors/transformers/foo-bar # Adjust path if needed
    docker compose --profile bdp up --build # Build and run your transformer
    ```

### Testing the Data Flow

  * **Check Transformer Logs**: Observe the logs of your `app` container.
  * **RabbitMQ Management**: Access `http://localhost:15673` (guest/guest) in your browser. Navigate to "Queues" and check the `ingress` queue (or whatever `MQ_EXCHANGE` you configured). You should see messages accumulating there.
